...
async function uploadTikTok(browser, videoPath, description) {
    // Cria uma nova página
    const page = await browser.newPage();

    // Carrega os cookies do arquivo
    console.log(' ○'.green + ' Carregando cookies...');
    const cookiesPath = path.join(process.cwd(), 'data', 'cookies_tiktok.json');
    ...
    try {
        ...
        if (!resultButtonPublish) {
            throw new Error('Falha ao publicar vídeo no TikTok');
        }
        ...
        if (!resultConfirmButtonPublish) {
            throw new Error('Falha ao confirmar publicação no TikTok');
        }
        ...
        console.log(' ○'.green + ' Vídeo publicado no TikTok com sucesso!'.white);

    } catch (error) {
        console.log(' ○'.red + ' Ocorreu um erro ao publicar vídeo no TikTok:', error.message);
    } finally {
        try {
            await page.close();
            console.log(' ○'.blue + ' Página do TikTok fechada com sucesso'.white);
        } catch { }
    }
