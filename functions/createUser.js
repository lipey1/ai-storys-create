...
async function createUser(browser) {
	try {
		...
		try {
			linkVerification = await new Promise((resolve, reject) => {
				setTimeout(() => {
					reject(new Error('Timeout ao buscar link de verificação'));
				}, 300000);
				const interval = setInterval(async () => {
					const inbox = await getInbox(address, password).catch((e) => {
						console.log(' ○'.red + ' Falha ao buscar inbox: '.white + e.message);
			...
			throw new Error('Falha ao buscar link de verificação: ' + error.message);
		}

		if (!linkVerification) {
			throw new Error('Falha ao buscar link de verificação');
		}
